...
class preprocess(base):
    ...
    def preprocessing_steps(self):
        # tokenize the text
        self.initial_tokens = [self.tokenizer.tokenize(text) for text in self.df[self.column]]
        
        # remove stopwords and substitute the unnecesary text to ""
        self.stopped_tokens = [[self.substitute(token).lower() for token in token_list if len(self.substitute(token).lower()) > 2 
                                and self.substitute(token).lower() not in self.stopwords] for token_list in self.initial_tokens]
    ...
    def paraphrase(self, list_of_paraphrasing: list=None, category: int=None):
        ...
        for index, row in temp_df.iterrows():
            ...
            for words in list_of_paraphrasing:
                paraphrased_text = re.sub(r'{}'.format(words[0]), words[1], paraphrased_text)
...
class eda:
    ...
    def generate_wordcloud(self, category: str = None, value: int = None):
        # if there is a column name given
        if category is not None:
            # if there are no values inputted, prompt the users to input a value
            if value is None:
